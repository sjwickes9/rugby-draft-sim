...
// CRITICAL initialization configuration lookup architecture block
...
function renderRosterList() {
    ...
    currentSpunSquad.forEach(player => {
        ...
        if (playerSelectedFromCurrentPool) row.classList.add("claimed-lockout");
...
pitchCircles.forEach(node => {
    node.addEventListener("click", () => {
        ...
        if (selectedPlayer.pos === "Back Row" && bPos === "Number 8" && !selectedPlayer.crossFunctionalBackRow) {
...
function populateManifestPreviewWindow() {
    ...
    for (let pos in userTeam) {
        ...
        tdPos.style.padding = "6px 4px"; tdPos.style.color = "var(--brand-gold)"; tdPos.textContent = pos;
        
        const tdName = document.createElement("td");
        tdName.style.padding = "6px 4px"; tdName.textContent = userTeam[pos].name;
        
        const tdScore = document.createElement("td");
        tdScore.style.padding = "6px 4px"; tdScore.style.textAlign = "right"; tdScore.textContent = userTeam[pos].score;
    ...
    runSimBtn.addEventListener("click", () => {
        ...
        function executeStep() {
            if (currentStep >= matches.length) {
                if (logs) logs.innerHTML += `<br><span class="sim-log-line" style="color:var(--brand-gold); font-weight:bold;">🏆 THE FINAL WHISTLE: YOU ARE THE WORLD CUP CHAMPIONS!</span>`;
            ...
            setTimeout(() => {
                ...
                if (logs) logs.innerHTML += `<span class="sim-log-line">FT: Drafted Hybrid XV ${userScore} - ${oppScore} ${m.opp}</span>`;
